...
num_examples = min([len(dataset['content']) for dataset in src_tgt_pairs.values()])
num_batches = num_examples / batch_size

STEP = 0
for epoch in range(start_epoch, config['training']['epochs']):
    if cur_metric >= best_metric and epoch % 100 == 99:
        # rm old checkpoint
    ...
    for i in range(0, num_examples, batch_size):
        ...
        src_idx = int((random.random() * 5) + 1)
        tgt_idx = int((random.random() * 4) + 1)
        tgt_idx += int(tgt_idx >= src_idx)
        src, tgt = src_tgt_pairs[("src", tgt_idx)], src_tgt_pairs[("tgt", tgt_idx)]
    ...
    #     print("We pass this part since we have no budget for MTURK")
